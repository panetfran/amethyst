...
actionsHTML += `<button class="meta-action-btn delete-btn" title="删除"><i class="fas fa-trash-alt"></i></button>`;
        ...
        const addMessage = (message) => {
            ...
            // 陪伴功能钩子：把这条消息同步给陪伴页面（记录到本次陪伴对话 + 显示气泡）
            try {
                if (message.sender === 'user') {
                    if (typeof window._onUserMessage === 'function') window._onUserMessage(message);
                } else if (message.sender !== 'system') {
                    if (typeof window._onPartnerMessage === 'function') window._onPartnerMessage(message);
                }
            } catch (e) { console.warn('[companion hook]', e); }
        };
