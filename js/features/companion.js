...
(function () {
    ...
    // ─── 补丁：把主聊天的新消息镜像进陪伴页面的气泡区 ──────────────────────
    // 陪伴页输入框发送时，实际上是把文字塞进主输入框再点主发送按钮（doCompanionSend），
    // 这样消息能正常存进 messages 数组、正常触发对方回复，但陪伴页自己的气泡 UI
    // 不会跟着刷新。这里通过包一层 renderMessages 来补上这个镜像。
    const _mirroredIds = new Set();

    function _seedMirroredIds() {
        // 进入陪伴页面时，把当前已有的消息都标记为"已镜像"，
        // 避免一进入就把历史消息当成新消息重新弹一遍气泡
        try {
            if (typeof messages !== 'undefined' && Array.isArray(messages)) {
                messages.forEach(m => { if (m && m.id !== undefined && m.id !== null) _mirroredIds.add(m.id); });
            }
        } catch (e) {}
    }

    function _installBubbleMirror() {
        if (typeof window.renderMessages !== 'function') {
            setTimeout(_installBubbleMirror, 500);
            return;
        }
        if (window.renderMessages._companionMirrorInstalled) return;

        const _origRenderMessages = window.renderMessages;

        window.renderMessages = function () {
            const result = _origRenderMessages.apply(this, arguments);
            try {
                const page = document.getElementById('companion-page');
                if (page && page.classList.contains('active') && typeof messages !== 'undefined' && Array.isArray(messages)) {
                    const recent = messages.slice(-6);
                    recent.forEach(function (m) {
                        if (!m || m.id === undefined || m.id === null) return;
                        if (_mirroredIds.has(m.id)) return;
                        if (m.type === 'system' || m.type === 'call-event' || m.sender === 'system') return;
                        if (!m.text && !m.image && !m.voice) return;
                        _mirroredIds.add(m.id);
                        showCompanionBubble(m);
                    });
                    // 避免 Set 无限增长
                    if (_mirroredIds.size > 300) {
                        const arr = Array.from(_mirroredIds).slice(-150);
                        _mirroredIds.clear();
                        arr.forEach(id => _mirroredIds.add(id));
                    }
                }
            } catch (e) {
                console.warn('[companion] 气泡镜像失败', e);
            }
            return result;
        };
        window.renderMessages._companionMirrorInstalled = true;
    }
