...
(function () {
    ...
    // ─── 群聊联动·邀请与日记专用：本次是谁邀请/陪伴了你 ──────────────────
    // 只影响邀请弹窗文案和日记记录，不影响陪伴页里的聊天气泡——
    // 气泡和对话历史走的是跟外面主聊天一样的"按消息ID换脸"逻辑（见下方
    // getBubbleActor），两套身份是分开的，邀请你的人不一定是里面说话的人。
    let _sessionActor = null;

    function rollSessionActor() {
        try {
            if (typeof groupChatSettings !== 'undefined' &&
                groupChatSettings && groupChatSettings.enabled &&
                Array.isArray(groupChatSettings.members) && groupChatSettings.members.length > 0) {
                const m = groupChatSettings.members[Math.floor(Math.random() * groupChatSettings.members.length)];
                _sessionActor = { name: m.name || '梦角', avatar: m.avatar || null };
            } else {
                _sessionActor = null;
            }
        } catch (e) {
    ...
    // 邀请文案 / 日记记录用这两个，不是 getPartnerName/getPartnerAvatarSrc
    function getSessionActorName() {
        return _sessionActor ? _sessionActor.name : getPartnerName();
    }
    function getSessionActorAvatar() {
        return _sessionActor ? (_sessionActor.avatar || null) : getPartnerAvatarSrc();
    }

    // ─── 群聊联动·气泡与对话历史专用：按消息ID换脸，跟外面主聊天同一套逻辑 ──
    function getBubbleActor(message) {
        try {
            if (typeof getGroupMemberForMessage === 'function' && message && message.id !== undefined) {
                const m = getGroupMemberForMessage(message.id);
                if (m) return { name: m.name || getPartnerName(), avatar: m.avatar || getPartnerAvatarSrc() };
            }
        } catch (e) {}
        return { name: getPartnerName(), avatar: getPartnerAvatarSrc() };
    }
    ...
    function showCompanionInviting(mode) {
        ...
        rollSessionActor();
        const partnerName = getSessionActorName();
        const avSrc = getSessionActorAvatar();
    ...
    async function showIncomingCompanion(mode) {
            ...
            try {
                if (typeof window.addCompanionDiaryEntry === 'function') {
                    ...
                    window.addCompanionDiaryEntry({
                        ...
                        userNote: '',
                        actorName: getSessionActorName(),
                        actorAvatar: getSessionActorAvatar()
                    });
                }
                _sessionActor = null; // 这次邀请流程结束（错过），身份状态清空，下次再抽
        ...
        overlay.querySelector('#companion-incoming-reject').addEventListener('click', () => {
            ...
            _sessionActor = null; // 这次邀请被拒绝，身份状态清空，下次再抽
        });
    ...
    function openCompanionPage(opts) {
        ...
        // 跟地图功能联动：陪伴开始的时候，把地图上"我"和"梦角"的位置都同步成"我的家·客厅"
        // （地图功能不一定存在，用 typeof 检查一下，没有就跳过，不影响陪伴本身）
        try {
            if (typeof window.mapSyncCompanionTogether === 'function') window.mapSyncCompanionTogether();
        } catch (e) {}
    ...
    function closeCompanionPage(opts) {
        ...
        _sessionActor = null; // 本场陪伴结束，身份状态清空，下次再重新抽
    ...
    function showCompanionBubble(message) {
        ...
        // 头像：对方一侧走群聊"按消息ID换脸"逻辑，跟外面主聊天保持一致
        const avSrc = isUser ? getMyAvatarSrc() : getBubbleActor(message).avatar;
        const avatarHtml = avSrc
            ? `<img src="${avSrc}">`
            : `<i class="fas fa-user"></i>`;
    ...
    function openCompanionHistory() {
        ...
        if (_sessionDialogue.length === 0) {
            ...
            listHtml = _sessionDialogue.map((m, idx) => {
                ...
                // 对方一侧按这条消息自己的id换脸，跟外面主聊天群聊逻辑一致
                const partnerAvSrcForThisMsg = getBubbleActor(m).avatar;
                const partnerAvatarHtmlForThisMsg = partnerAvSrcForThisMsg ? `<img src="${partnerAvSrcForThisMsg}">` : `<i class="fas fa-user"></i>`;
                const avatarHtml = isUser ? userAvatarHtml : partnerAvatarHtmlForThisMsg;
    ...
    async function handleMgrBgUpload(e) {
        ...
        try {
            ...
            if (cloudReady) {
                ...
                // 没连云端：降级为 base64 存进 localforage（跟语音上传逻辑保持一致）
                // 注意：视频文件转 base64 体积会很大，大视频建议还是走云端
                placeholder._progress = 30;
                _updateCardProgress(30);
                mediaData = await readFileAsBase64(file);
                placeholder._progress = 90;
                _updateCardProgress(90);
            }
