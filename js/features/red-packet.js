...
(function () {
    ...
    window.trySystemRedPacket = function (forceChance) {
        window.initTransferData();
        ...
        var festivals = getFestivals();
        var festival = festivals.length > 0 ? festivals[0] : null;
        // forceChance 由定时器等外部调用传入时，优先使用；否则按节日/平日走默认概率
        var chance = typeof forceChance === 'number' ? forceChance : (festival ? 0.85 : 0.25);
    ...
    function _scheduleAutoRedPacket() {
        // 每次都重新随机一个 2~3 小时的间隔，而不是固定周期
        var delay = (2 + Math.random()) * 60 * 60 * 1000; // 2h ~ 3h
        setTimeout(function () {
            try {
                window.trySystemRedPacket(0.5); // 强制用 50% 概率判定
            } catch (e) {
                console.warn('[RedPacket] 自动判定出错:', e);
            }
            _scheduleAutoRedPacket(); // 安排下一次判定
        }, delay);
    }

    // 页面加载后延迟一点再启动（等 transferData / 聊天数据加载完成）
    setTimeout(_scheduleAutoRedPacket, 10000);
