...
(function() {
    ...
    window.addCompanionDiaryEntry = async function(entry) {
        ...
        const rec = {
            ...
            actorName: entry.actorName || null,   // 群聊联动：这次陪伴的是谁（不开群聊则为null）
            actorAvatar: entry.actorAvatar || null
        };
        ...
        months.forEach(mo => {
            ...
            if (entries.length === 0) {
                ...
                entries.forEach(e => {
                    ...
                    // 群聊联动：这条记录当时是跟谁陪伴的，优先用记录自己存的身份，
                    // 没有存（说明当时群聊没开，或者是老记录）就用现在的对方名字兜底
                    const entryPartnerName = e.actorName || partnerName;

                    const isMissed = !!e.missed;
                    const initiatorLabel = isMissed
                        ? (entryPartnerName + '邀请')
                        : (e.initiator === 'partner' ? (entryPartnerName + '邀请') : (userName + '邀请'));
                    ...
                    const partnerRowHtml = hasPartnerNote
                        ? '<div class="cd-note-row">' +
                            '<span class="cd-note-who">' + escapeHtml(entryPartnerName) + '：</span>' +
                            '<span class="cd-note-text">' + escapeHtml(e.partnerNote) + '</span>' +
                          '</div>'
                        : '<div class="cd-note-row">' +
                            '<span class="cd-note-empty">' + escapeHtml(entryPartnerName) + '没有记录</span>' +
                          '</div>';
