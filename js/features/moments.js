/**
 * features/moments.js - 朋友圈功能（无 AI 版本）
 *
 * 依赖（均已存在于现有代码中）：
 *   getStorageKey, safeGetItem, safeSetItem, showNotification,
 *   settings, customReplies, addMessage, showModal, hideModal, SESSION_ID
 *
 * 与原版的区别：原版有"展开AI解读"功能（把字卡拼出来的短句用AI扩写成
 * 自然语句），这一版整个去掉了，评论/回复/发帖内容就是直接从你的
 * 字卡库（customReplies）里随机抽1~3条拼起来，跟你现有的模拟回复
 * 系统（simulateReply）用的是同一套逻辑，不调用任何 AI 接口。
 */
...
(function() {
    ...
    function loadMomentsData() {
        try {
            const saved = safeGetItem(getStorageKey('momentsData'));
            if (saved) {
            ...
            momentsData = { myCover: null, partnerCover: null, partnerCoverOptions: [], partnerImagePool: [], posts: [] };
        }
    }

    function saveMomentsData() {
        safeSetItem(getStorageKey('momentsData'), JSON.stringify(momentsData));
    }

    // ─── 随机字卡文案（跟 simulateReply 用同一个池子）──────
    ...
    window.openPartnerImagePool = function() {
        ...
        html += '</div>';
        html += '<div style="font-size:11px;color:var(--text-secondary);padding:8px 0;text-align:center;">这些图片会被随机用于对方的自动发帖配图</div>';
    ...
    window.addPartnerPoolItem = function() {
        const mode = confirm('点击"确定"选择本地图片，点击"取消"使用图片链接');
        const addItem = (url) => {
            momentsData.partnerImagePool.push({ id: 'pool_' + Date.now(), url: url });
            saveMomentsData();
            window.openPartnerImagePool();
