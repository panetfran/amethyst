...
(function() {
    ...
    let momentsLastSeenAt = 0;     // 角标未读：上次点开朋友圈面板的时间戳
    ...
    // ─── 角标：统计"对方发的、你还没看过"的内容数量 ──────────
    function computeUnreadCount() {
        let count = 0;
        momentsData.posts.forEach(post => {
            if (post.author === 'partner' && post.timestamp > momentsLastSeenAt) count++;
            (post.comments || []).forEach(c => {
                if (c.author === 'partner' && c.timestamp > momentsLastSeenAt) count++;
                (c.replies || []).forEach(r => {
                    if (r.author === 'partner' && r.timestamp > momentsLastSeenAt) count++;
                });
            });
        });
        return count;
    ...
    function refreshBadge() {
        if (typeof window.appBadges !== 'undefined') {
            window.appBadges.set('moments-function', computeUnreadCount());
        }
    }

    async function markMomentsSeen() {
        momentsLastSeenAt = Date.now();
        safeSetItem(getStorageKey('momentsLastSeenAt'), String(momentsLastSeenAt));
        refreshBadge();
    }
    ...
    async function loadMomentsData() {
        ...
        try {
            const seenRaw = safeGetItem(getStorageKey('momentsLastSeenAt'));
            momentsLastSeenAt = seenRaw ? parseInt(seenRaw, 10) || 0 : 0;
        } catch (e) { momentsLastSeenAt = 0; }
        refreshBadge();
    }
        ...
        if (posts.length === 0) {
            ...
            posts.forEach(post => {
                ...
                const commentCount = post.comments
                    ? post.comments.reduce((sum, c) => sum + 1 + (c.replies ? c.replies.length : 0), 0)
                    : 0;
    ...
    // ─── 对方自动点赞 + 评论（发帖后）：两件事独立判定，不再绑在一起 ──────
    function schedulePartnerReaction(postId) {
        // 点赞：75% 概率会点，30秒~6分钟内点
        if (Math.random() < 0.75) {
            setTimeout(() => {
        ...
        // 评论：55% 概率会评论，1分钟~10分钟内评论，跟点赞是否发生完全无关
        if (Math.random() < 0.55) {
            setTimeout(() => {
                const post = momentsData.posts.find(p => p.id === postId);
                if (!post) return;
    ...
    window.commentOnMoment = function(postId) {
        ...
        if (Math.random() < 0.8) {
        const delay = randomDelay(20 * 1000, 4 * 60 * 1000);
        setTimeout(() => {
    ...
    // ─── 对方自动发朋友圈：每日随机调度 ──────
    function initPartnerMomentsSchedule() {
        ...
        // 不管是刚生成的还是之前已经存在的，统一在这里设置定时器——只走这一处，
        // 不会再出现"生成时设一次、恢复时又设一次"导致同一个时间点触发两次的问题
        if (!schedule.posted) schedule.posted = [];
        schedule.times.forEach(t => {
            if (schedule.posted.includes(t)) return;
            if (t < Date.now() - 60 * 60 * 1000) return; // 超过1小时的错过时段不再补发，避免堆积
            const delay = Math.max(0, t - Date.now());
            setTimeout(() => {
                publishPartnerMoment();
                if (!schedule.posted.includes(t)) schedule.posted.push(t);
                safeSetItem(getStorageKey('momentsSchedule'), JSON.stringify(schedule));
            }, delay);
        });
    }
    ...
    function publishPartnerMoment() {
        ...
        // 小概率对方给自己发的这条点个赞（自恋小彩蛋），一条帖子最多算一次
        if (Math.random() < 0.2) {
            post.likes.push('partner');
        }
