...
const PARTNER_LEAVE_INTERVAL_MIN = 60 * 60 * 1000;  // 1小时
const PARTNER_LEAVE_INTERVAL_MAX = 120 * 60 * 1000; // 2小时
...
function scheduleBoardCheck() {
    ...
    // 每隔15~20分钟检查一次；真正要不要生成新留言，由函数内部的
    // 1~2小时随机冷却 + 50%概率 这两道门限来把控节奏，
...
function checkAndGeneratePartnerMsg() {
    ...
    Promise.all([
        ...
        // 冷却时间在1~2小时之间随机，不是每次都卡死同一个数字，间隔感更自然
        const requiredInterval = PARTNER_LEAVE_INTERVAL_MIN + Math.random() * (PARTNER_LEAVE_INTERVAL_MAX - PARTNER_LEAVE_INTERVAL_MIN);
