...
function deduplicateContentArray(arr, baseSystemArray = []) {
    ...
    return { result, removedCount };
        ...
        // 当前正在播放的邀请提示音（invite_xxx），用于同意/拒绝时立即打断
        let _currentInviteAudio = null;
        window.stopCurrentSound = function () {
            if (_currentInviteAudio) {
                try { _currentInviteAudio.pause(); _currentInviteAudio.currentTime = 0; } catch (e) {}
                _currentInviteAudio = null;
            }
        };
        ...
        const playSound = (type) => {
            ...
            try {
                // 陪伴功能的邀请提示音：invite_study / invite_work / invite_exercise /
                // invite_sleep / invite_videocall，播放 assets/audio/ 下的真实 mp3 文件，
                // 不走下面的合成音效逻辑
                if (typeof type === 'string' && type.indexOf('invite_') === 0) {
                    window.stopCurrentSound(); // 保险起见，先停掉上一段还没放完的邀请音
                    const audio = new Audio('assets/audio/' + type + '.mp3');
                    audio.volume = Math.min(1, Math.max(0, settings.soundVolume || 0.15));
                    _currentInviteAudio = audio;
                    audio.play().catch(() => {}); // 文件不存在或加载失败时静默跳过，不报错打断陪伴功能
                    return;
                }
