...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    // 暴露一个精简的公共接口，供"一起听歌邀请"这类外部功能模块调用
    // （播放器内部这些变量/函数都是这个函数自己作用域里的，外面本来拿不到）
    window.musicPlayerAPI = {
        ...
        isEnabled() {
            return !!settings.musicPlayerEnabled;
        },
        getSongCount() {
            return songs.length;
        },
        ...
        playRandom() {
            if (songs.length === 0) return false;
            return this.playIndex(Math.floor(Math.random() * songs.length));
        },
        getCurrentSongTitle() {
            if (songs.length === 0) return null;
            return songs[currentIndex] ? songs[currentIndex].title : null;
        }
    };
};
